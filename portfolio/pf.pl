...
#
my $cookiename="PfSession";
#
# And another cookie to preserve the debug state
#
my $debugcookiename="PfDebug";
...
if ($action eq "login") { 
  if ($run) { 
    ...
    #
    ($user,$email,$password) = (param('user'),param('email'),param('password'));
    if (ValidUser($user,$email,$password)) { 
      ...
      $outputcookiecontent=join("/",$user,$email,$password);
      $action = "base";
      $run = 1;
    } else {
...
print "<head>";
print "<title>My Portfolios</title>";
print "</head>";
...
if ($action eq "login") { 
  ...
  if ($logincomplain or !$run) { 
    print start_form(-name=>'Login'),
      h2('Login to your portfolios'),
	"Name:",textfield(-name=>'user'),	p,
    "Email:", email_field(-name=> 'email'), p,
...
sub UserAdd { 
  eval { ExecSQL($dbuser,$dbpasswd,
		 "insert into users (name, email, password) values (?,?,?)",undef,@_);};
  return $@;
}
...
sub ValidUser {
  ...
  my @col;
  eval {@col=ExecSQL($dbuser,$dbpasswd, "select count(*) from users where name=? and email=? and password=?","COL",$user,$email,$password);};
  if ($@) { 
    return 0;
  } else {
    return $col[0]>0;
